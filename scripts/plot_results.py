...
class Plotter():
    ...
    def add_plot(self, x, y, label, linecolour, linestyle):
        ...
        assert (len(self.y) == len(self.x) and len(self.x) == len(self.labels)
                and len(self.labels) == len(self.linecolors)
                and len(self.linecolors) == len(self.linestyle))
...
def get_accs(expdir, timesteps):
    ...
    for i in range(timesteps):
        try:
            labels1, preds1 = np.load(expdir + '/labels_' + str(i + 1) +
                                      '_pretestset.npy'), np.load(
                                          expdir + '/preds_' + str(i + 1) +
                                          '_pretestset.npy')
            acc = ((labels1 == preds1) * 1.0).mean() * 100
            pretestacc.append(acc)
            labels2, preds2 = np.load(expdir + '/labels_' + str(i + 1) +
                                      '_cltestset.npy'), np.load(
                                          expdir + '/preds_' + str(i + 1) +
                                          '_cltestset.npy')
            acc = ((labels2 == preds2) * 1.0).mean() * 100
            cltestacc.append(acc)
            labelsconcat, predsconcat = np.concatenate((labels1, labels2),
            ...
            continue
    pretestacc, cltestacc, totacc = np.array(pretestacc), np.array(
        cltestacc), np.array(totacc)
...
plter.add_plot(x=x,
               y=preacc,
               label='Uniform -- 3000 Iters Optimal -- (Im1K)',
               linecolour=linecolours[6],
               linestyle='dashed')
plter.add_plot(x=x,
               y=clacc,
               label='Uniform -- 3000 Iters Optimal -- (CL)',
               linecolour=linecolours[6],
               linestyle='solid')
plter.add_plot(x=x,
               y=totacc,
               label='Uniform -- 3000 Iters (Total)',
               linecolour=linecolours[6],
               linestyle='dotted')
