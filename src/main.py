...
from training import save_representations, test, train
from utils import LinearLR, get_logger, save_model, seed_everything
...
def per_timestep_loop(opt, logger, dataset):
    ...
    if opt.fc == 'cosine_linear':
        model.fc = CosineLinear(in_features=linear_size,
                                out_features=opt.num_curr_classes)
    else:
    ...
    if opt.pretrain_modelpath is not None:
        logger.info('==> Loading from ' + opt.pretrain_modelpath +
    ...
    if opt.expand_size > 0:  # Add new classes to the final classifier
        ...
        model.fc.weight = nn.Parameter(
            torch.cat((model.fc.weight, new_weights), dim=0))
        if model.fc.bias is not None:
            new_biases = torch.zeros(opt.expand_size)
            model.fc.bias = nn.Parameter(
                torch.cat((model.fc.bias, new_biases), dim=0))
    ...
    model, optimizer, scheduler = train(opt=opt,
    ...
    if opt.dset_mode == 'class_incremental' and opt.calibrator != None:
        model = correct_weights(model=model,
                                valloader=dataset.valloader,
                                calibration_method=opt.calibrator,
                                logger=logger,
                                expand_size=opt.expand_size)

    if opt.sampling_mode in ['herding', 'kmeans']:
        save_representations(opt=opt,
                             loader=dataset.trainloader_eval,
                             model=model,
                             mode='features')

    elif opt.sampling_mode in ['unc_lc', 'max_loss']:
        save_representations(loader=dataset.trainloader_eval,
                             model=model,
                             mode='predictions')

    # Testing Part
    predsarr, labelsarr = test(loader=dataset.testloader,
                               model=model,
                               logger=logger)
    np.save(
        opt.log_dir + '/' + opt.exp_name + '/labels_' + str(opt.timestep) +
        '_cltestset.npy', labelsarr.numpy())
    np.save(
        opt.log_dir + '/' + opt.exp_name + '/preds_' + str(opt.timestep) +
        '_cltestset.npy', predsarr.numpy())
    del predsarr, labelsarr

    if opt.dataset == 'Imagenet2K':
        predsarr, labelsarr = test(loader=dataset.pretestloader,
                                   model=model,
                                   logger=logger)
        np.save(
            opt.log_dir + '/' + opt.exp_name + '/labels_' + str(opt.timestep) +
            '_pretestset.npy', labelsarr.numpy())
        np.save(
            opt.log_dir + '/' + opt.exp_name + '/preds_' + str(opt.timestep) +
            '_pretestset.npy', predsarr.numpy())
        del predsarr, labelsarr
...
if __name__ == '__main__':
    ...
    torch.multiprocessing.set_sharing_strategy(
        'file_system')  # For error: Too many files open
    opt = parse_args()
    ...
    for timestep in range(opt.num_timesteps):
        ...
        if opt.model_type == 'gdumb' or opt.timestep == 1:
            ...
            opt.pretrain_modelpath = opt.log_dir + '/' + opt.exp_name + '/' + str(
                opt.timestep - 1) + '/last.ckpt'
        ...
        if (not exists(opt.log_dir + '/' + opt.exp_name + '/' +
                       str(opt.timestep + 1) + '/last.ckpt')):
            console_logger.info('==> Starting training of timestep ' +
                                str(opt.timestep) + '..')
            per_timestep_loop(opt=opt, logger=console_logger, dataset=dataset)
            console_logger.info('==> Completed training for timestep ' +
                                str(opt.timestep) + '..')
        opt.num_curr_classes = len(dataset.curr_classes)

    console_logger.info('Experiment completed! Total timesteps: ' +
                        str(opt.timestep))
