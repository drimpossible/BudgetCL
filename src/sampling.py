    ...
    if opt.timestep>1:
        if not class_balanced:
            if opt.sampling_mode in ['herding', 'kmeans']:
                feats = np.load(opt.log_dir+'/'+opt.exp_name+'/feats_'+str(opt.timestep-1)+'_train.npy')
                labels = np.load(opt.log_dir+'/'+opt.exp_name+'/labels_'+str(opt.timestep-1)+'_train.npy')
                ...
                probs, labels = np.load(opt.log_dir+'/'+opt.exp_name+'/predprobs_'+str(opt.timestep-1)+'_train.npy'), np.load(opt.log_dir+'/'+opt.exp_name+'/labels_'+str(opt.timestep-1)+'_train.npy')
                sampled_idxes = samplewise_losses(opt, probs, labels, num_samples)
        else:
            labels = np.load(opt.log_dir+'/'+opt.exp_name+'/labels_'+str(opt.timestep-1)+'_train.npy')
            if opt.sampling_mode in ['herding', 'kmeans']:
                feats = np.load(opt.log_dir+'/'+opt.exp_name+'/feats_'+str(opt.timestep-1)+'_train.npy')
            elif opt.sampling_mode in ['unc_lc','max_loss']:
                probs = np.load(opt.log_dir+'/'+opt.exp_name+'/predprobs_'+str(opt.timestep-1)+'_train.npy')
            
            classweights = np.bincount(np.array(labels))
            num_samples_per_class = (num_samples//len(classweights))+1 # If not divisible, then store slightly more than num_samples
            sampled_idxes = []
            for cls in len(classweights):
                idx = np.where(labels==cls)[0]
                if idx.shape[0] < num_samples_per_class: # Can't do anything but undersample
                    ...
                    if opt.sampling_mode == 'herding':
                        ...
                        cls_idx = samplewise_losses(opt, probs[idx], labels[idx], num_samples_per_class)
...
def samplewise_losses(opt, probs, labels, num_samples):
    if opt.sampling_mode == 'unc_lc':
        ...
        prob_gt = np.array([probs[j, labels[j]] for j in range(probs.shape[0])])
...
def herding(features, nb_examplars):
    ...
    herding_matrix[np.where(herding_matrix == 0)[0]] = 100000000 # Some high number
