...
class BiC(nn.Module):
    ...
    def __init__(self, num_new_cls):
        ...
        self.alpha, self.beta = nn.Parameter(torch.ones(1) * 0.75), nn.Parameter(torch.ones(1) * 0.001)
...
def correct_weights(model, valloader, calibration_method, logger, expand_size):
    ...
    if calibration_method == 'WA':
        ...
        gamma = model.fc.weight[:extra].norm(p=2, dim=1).mean()/model.fc.weight[extra:].norm(p=2, dim=1).mean()
        with torch.no_grad(): 
            intermediate = model.fc.weight[extra:]*gamma 
            model.fc.weight[extra:] = nn.Parameter(intermediate)
            if model.fc.bias is not None:
                model.fc.bias[extra:] = nn.Parameter(model.fc.bias[extra:]*gamma)
    ...
    # Weights corrected according to: Large Scale Incremental Learning (https://arxiv.org/abs/1905.13260) 
        ...
        for lr_exp in range(2,4):
            ...
            for epoch in range(5): 
                calibrator, calib_optimizer = calibrate(loader=valloader, model=model, calibrator=calibrator, optimizer=calib_optimizer, logger=logger, epoch=epoch)
        ...
        with torch.no_grad():
            ...
            model.fc.weight[extra:] = nn.Parameter(intermediate)
            if model.fc.bias is not None:
                model.fc.bias[extra:] = nn.Parameter(model.fc.bias[extra:]*calibrator.alpha + calibrator.beta)
    ...
    # Baseline weight correction according to: On Calibration of Modern Neural Networks (https://arxiv.org/abs/1706.04599)
        ...
        model.fc.weight = nn.Parameter(model.fc.weight/calibrator.temperature)
        if model.fc.bias is not None:
            model.fc.bias = nn.Parameter(model.fc.bias/calibrator.temperature)
...
def calibrate(loader, model, calibrator, optimizer, logger, epoch):
    ...
    progress = ProgressMeter(logger,
        ...
        prefix="Epoch: [{0}] LR: [{1:.4f}]".format(epoch, optimizer.param_groups[0]['lr']))
