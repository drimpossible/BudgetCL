...
class CLImageFolder(object):
    ...
    def __init__(self, opt):
        ...
        self.kwargs = {'num_workers': opt.num_workers, 'batch_size': opt.train_batch_size, 'pin_memory': True, 'shuffle': False, 'drop_last': False}
        self.opt = opt
        ...
        if opt.dataset == 'Imagenet2K':
            ...
            self._pretrain_paths, self._preval_paths, self._pretest_paths, self._pretrain_labels, self._preval_labels, self._pretest_labels = self.populate_paths(trainpath=opt.order_file_dir+'/pretrain.txt', valpath=opt.order_file_dir+'/preval.txt', testpath=opt.order_file_dir+'/pretest.txt')
            self.curr_classes = copy.deepcopy(list(self._classes.values())) # At timestep 0, curr_paths is pretrain_paths

            assert(opt.dset_mode in ['class_incremental', 'data_incremental'])

            self._cltrain_paths, self._clval_paths, self._cltest_paths, self._cltrain_labels, self._clval_labels, self._cltest_labels = self.populate_paths(trainpath=opt.order_file_dir+'/ordering_'+opt.dset_mode+'.txt', valpath=opt.order_file_dir+'/val.txt', testpath=opt.order_file_dir+'/test.txt')
            ...
            self._classes = {}
            self._cltrain_paths, _, self._test_paths, self._cltrain_labels, _, self._test_labels = self.populate_paths(trainpath=opt.order_file_dir+'/train.txt', valpath=opt.order_file_dir+'/test.txt', testpath=opt.order_file_dir+'/test.txt')
            self.curr_classes = copy.deepcopy(list(self._classes.values())) # At timestep 0, curr_paths is pretrain_paths
            ...
            self.testloader = DataLoader(ListFolder(imgpaths=copy.deepcopy(self._test_paths), labels=self._test_labels, transform=ClassificationPresetEval(crop_size=self.opt.crop_size, resize_size=self.opt.crop_size+32)), **self.kwargs)
    ...
    def populate_paths(self, trainpath, valpath, testpath):
        ...
        for i in range(len(trainimgs)):
            if self.opt.dataset == 'Imagenet2K':
                ...
                if clsname not in self._classes: self._classes[clsname] = len(self._classes)
    ...
    def get_loader(self, opt, paths, labels, train=True):
        ...
        if train:
            return DataLoader(ListFolder(data_dir= opt.data_dir, imgpaths=paths, labels=labels, transform=ClassificationPresetTrain(crop_size=self.opt.crop_size)), **self.kwargs)
        else:
    ...
    def get_next_timestep_dataloader(self,opt):
        ...
        assert(self.opt.dset_mode in ['class_incremental', 'data_incremental'])
        if self.opt.dset_mode == 'class_incremental':
            assert(self.opt.increment_size==0)
            increment_size, start = 0, (self.opt.timestep-1)*self.opt.num_classes_per_timestep
            for i in range(self.opt.num_classes_per_timestep): increment_size += self.class_sizes[start+i]
        else:
            assert(self.opt.num_classes_per_timestep == 0)
            increment_size = self.opt.increment_size
            
        new_paths = self._cltrain_paths[self.current_sample:self.current_sample+increment_size]
        new_labels = self._cltrain_labels[self.current_sample:self.current_sample+increment_size]
    ...
    def sample_data(self):
        if len(self.curr_paths) < (self.opt.total_steps*self.opt.train_batch_size): # Edge case-- if sampling size is higher than the all stored data then we don't select, simply add all samples. Won't be the case in most experiments.
            ...
            curr_paths += copy.deepcopy(self.curr_paths[len(self.curr_paths)-(remaining):])
            curr_labels += copy.deepcopy(self.curr_labels[len(self.curr_paths)-(remaining):])
        else:
            if self.opt.sampling_mode == 'recency_biased':
                ...
                for idx in range((self.opt.total_steps*self.opt.train_batch_size), len(self.curr_paths)):
                    dice = random.randint(0,idx)
                    if dice < (self.opt.reservoir_alpha*self.opt.total_steps*self.opt.train_batch_size):
                ...
                weights = np.array([self.classweights[self.curr_labels[i]].item() for i in range(len(self.curr_labels))])
...
class ListFolder(Dataset):
    def __init__(self, data_dir, imgpaths, labels, transform=None, return_paths=False):
    ...
    def __len__(self):
        assert(len(self.image_paths)==self.labels.size(0)), 'Length of image path array and labels different'
